...
n_sim = 10
unlabel_size_list = np.concatenate((([50, 150] ,75 * np.arange(3, 15, 2))))
...
def semi_supervised_simulation(unlabel_size):
    ...
    if os.path.exists(f'./result/semi_supervised_improve/unlabel_size_{unlabel_size}.pkl'):
        score_table_t = pd.read_pickle(f'./result/semi_supervised_improve/unlabel_size_{unlabel_size}.pkl')
    else:
    ...
    for i_sim in range(n_sim):
            ...
            while 1:
                Loss = torch.sum(- y.unsqueeze(1) * torch.mm(X, beta_h) + torch.log(1 + torch.exp(torch.mm(X, beta_h)))) + \
                       1/(1 + alpha) * (
                        ...
                        alpha * n_label/n_unlabel *lamb/2 * delta/(1 - delta) * torch.sum(A_unlab.squeeze(1) * torch.sum(X_unlab**2 * (beta_h**2).T, axis=1)))
        ...
        score_table_t.to_pickle(f'./result/semi_supervised_improve/unlabel_size_{unlabel_size}.pkl')
...
score_table.to_pickle('./result/semi_supervised_improve/score_table_over_unlabel_size.pkl')
...
plt.savefig('./result/semi_supervised_improve/Semi_supervised_learning.png')
plt.show()
